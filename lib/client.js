...
const { EventTypes, ChatActions, APP_VERSION, BUILD_NUMBER } = require('./constants');
...
class WebMaxClient extends EventEmitter {
  constructor(options = {}) {
    ...
    const webDefaults = {
      ...
      // Ниже актуальной версии сервер отвечает «Приложение устарело» / qr_login.disabled.
      // Не берём appVersion/buildNumber из session — старые сессии иначе снова шлют outdated fingerprint.
      appVersion: options.appVersion || configObj.appVersion || APP_VERSION,
      screen:
        options.screen ||
        configObj.screen ||
      ...
      buildNumber: options.buildNumber ?? configObj.buildNumber ?? (deviceType === 'ANDROID' ? BUILD_NUMBER : undefined),
  ...
  async getUser(userId) {
    const response = await this.sendAndWait(Opcode.CONTACT_INFO, {
      contactIds: [userId]
    });

    const contact =
      response.payload && response.payload.contacts && response.payload.contacts.length > 0
        ? response.payload.contacts[0]
        : null;

    return this._contactToUser(contact);
  }

  /**
   * Нормализация номера: '+79001234567'.
   */
  ...
  /**
   * Contact → User (общий парсер для CONTACT_INFO / CONTACT_INFO_BY_PHONE).
   */
  _contactToUser(contact) {
    ...
    return new User({
      ...
      firstname: name.firstName || name.name || '',
      lastname: name.lastName || '',
      username: contact.nick || name.nick || null,
      phone: contact.phone,
      avatar: contact.baseUrl || contact.baseRawUrl,
      photoId: contact.photoId,
  ...
  /**
   * Поиск пользователя по номеру телефона (opcode CONTACT_INFO_BY_PHONE = 46).
   * Работает, если у цели в приватности разрешён поиск по телефону.
   * @returns {Promise<User|null>}
   */
  async getUserByPhone(phone) {
    const normalizedPhone = this._normalizePhoneNumber(phone);
    try {
      const response = await this.sendAndWait(Opcode.CONTACT_INFO_BY_PHONE, {
        phone: normalizedPhone
      });
      ...
      if (/contact\.not\.found|не найден|not found/i.test(msg)) {
        return null;
      }
      throw e;
    }
  ...
  /**
   * Завершить/отклонить звонок через OK API:
