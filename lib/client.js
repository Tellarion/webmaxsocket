...
class WebMaxClient extends EventEmitter {
  ...
  _applyLoginTokenFromSyncResponse(responsePayload) {
    const newTok = responsePayload?.tokenAttrs?.LOGIN?.token;
    if (!newTok || typeof newTok !== 'string') {
      return;
    }
    const trimmed = newTok.trim();
    if (!trimmed) {
      return;
    }
    ...
    this._token = trimmed;
    if (this._saveTokenToSession) {
      this.session.set('token', trimmed);
    }
    if (this.debug || this._authDebug) {
      console.log('[webmaxsocket:auth] Сохранён новый токен из ответа LOGIN (sync)');
    }
  }
  ...
  async closeAllSessionsExceptCurrent() {
    ...
    const times = rows.filter((r) => !r.current && r.time > 0).map((r) => r.time);
    const attempts = [
      { allExceptCurrent: true },
      { closeAllExceptCurrent: true },
      { excludeCurrent: true },
      ...(times.length
        ? [{ times }, { sessionTimes: times }, { ids: times }]
        : [])
    ];
    let lastErr;
